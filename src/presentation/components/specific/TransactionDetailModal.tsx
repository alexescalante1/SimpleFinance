// PRESENTATION - TransactionDetailModal.tsx
...
import { validateTransactionDetail, calculateDetailDiscrepancy } from '@/domain/valueObjects/TransactionVo';
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <View style={{ flex: 1, marginRight: 8 }}>
            <Controller
              ...
              rules={{
                validate: (value) => {
                  const error = validateTransactionDetail({ amount: parseFloat(value) || 0, description: 'temp' });
              ...
              render={({ field: { onChange, onBlur, value } }) => (
                <View style={{ marginBottom: 8 }}>
                  <TextInput
                    ...
                    placeholder="0.00"
                  />
                  <HelperText type="error" visible={!!errors?.details?.[index]?.amount}>
            ...
            <Controller
              ...
              rules={{
                validate: (value) => {
                  const error = validateTransactionDetail({ amount: 1, description: value });
              ...
              render={({ field: { onChange, onBlur, value } }) => (
                <View>
                  ...
                  />
                  <HelperText type="error" visible={!!errors?.details?.[index]?.description}>
...
export const TransactionDetailModal: React.FC<TransactionDetailModalProps> = ({
  ...
  const themeColors = {
    ...
    successBackground: theme.dark ? 'rgba(76, 175, 80, 0.15)' : '#E8F5E8',
  ...
  useEffect(() => {
    if (visible && transaction) {
      const initialDetails = transaction.detail?.length > 0 
  ...
  const onSubmit = async (data: DetailFormData) => {
    ...
    try {
      ...
      if (calculations?.hasDiscrepancy) {
        const discrepancyDetail = calculateDetailDiscrepancy(transaction.amount, validDetails);
              ...
              <Card mode="outlined" style={{ marginBottom: 20 }}>
                <Card.Content>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                    ...
                    <Text variant="bodyMedium" style={{ color: themeColors.text, fontWeight: '500' }}>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    ...
                    <Text variant="bodyMedium" style={{ 
                      ...
                      {calculations.hasDiscrepancy 
                        ? `S/ ${Math.abs(calculations.discrepancy).toFixed(2)} ${calculations.discrepancyType === 'positive' ? '(faltante)' : '(sobrante)'}`
                        : 'Sin diferencia ✓'
                      }
                    </Text>
                  </View>
